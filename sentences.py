...
def verif(cont, text):
	...
	for i in range(cont, len(text)):
		if(auxSense==""):	senses= dbc.senseSearch(text[i])
		else:			senses= dbc.senseSearch(auxSense+"_" +text[i])
		if(len(senses)==0): return auxSense,i-1
		if(auxSense==""):	auxSense= text[i]
		else:			auxSense= auxSense+"_" +text[i]
...
def procSentence(text):
	...
	for j in range(i,counter):	vis[j]=1
